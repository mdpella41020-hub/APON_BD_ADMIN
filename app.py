    ...
    <style>
        * {
        ...
        .banner-overlay {
            position: absolute;
            inset: 0;
            background: linear-gradient(to bottom, transparent 0%, #020503 100%);
        }
        ...
        .view-more-btn {
            background: linear-gradient(135deg, #00ff66 0%, #003311 100%);
        ...
        .animate-fade-in-up {
            animation: fadeInUp 0.4s cubic-bezier(0.15, 0.85, 0.35, 1) forwards;
        }
        @keyframes fadeInUp {
        ...
        .toast {
            background: rgba(4, 12, 6, 0.95); border: 1px solid rgba(57, 255, 20, 0.3); border-radius: 10px;
            padding: 0.7rem 1.2rem; font-size: 0.85rem; color: #ecffed;
        ...
        .cyber-paste-btn {
            background: linear-gradient(135deg, rgba(0, 255, 204, 0.15) 0%, rgba(0, 114, 255, 0.15) 100%);
            border: 1px solid rgba(0, 255, 204, 0.4);
            color: #00ffcc;
            text-shadow: 0 0 8px rgba(0, 255, 204, 0.5);
            transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);
        }
        .cyber-paste-btn:hover {
            background: linear-gradient(135deg, #00ffcc 0%, #0072ff 100%);
            color: #000000;
            text-shadow: none;
            box-shadow: 0 0 15px rgba(0, 255, 204, 0.6);
            transform: scale(1.02);
        }
        .cyber-paste-btn:active {
...
<body class="py-6 px-4 max-w-xl mx-auto flex flex-col justify-start selection:bg-green-500 selection:text-black">
    ...
    <script>
        ...
        function filterImageUrl(url) {
            if (!url || url === PLACEHOLDER_URL || url.includes('via.placeholder.com')) {
                        ...
                        <div class="flex items-center gap-2 shrink-0 flex-wrap justify-end w-full sm:w-auto mt-2 sm:mt-0">
                            ...
                            <div class="w-full text-right mt-1.5 flex justify-end">
                                <span class="bg-black/60 px-2.5 py-1 rounded-md text-[11px] text-cyberGreen font-mono font-bold tracking-widest border border-green-500/20 shadow-[0_0_5px_rgba(57,255,20,0.2)]">
                                    <i class="fa-solid fa-clock mr-1 text-xs"></i><span id="uptime-${uid}">0d 0h 0m</span>
                                </span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div id="expanded-block-${uid}" class="hidden space-y-4 mt-2 transition-all duration-300">
                        ...
                        <div class="glass-card rounded-2xl relative overflow-hidden pb-4">
                            <div class="banner-wrapper">
                                <div id="bannerBg-${uid}" class="banner-bg" style="background-image: url('${DEFAULT_IMG_URL}')"></div>
                                <div class="banner-overlay"></div>
                            </div>
                            
                            <div class="basic-info">
                                <div class="avatar-frame w-[100px] h-[100px] mt-[-50px] mb-2 relative">
                                    <img id="pAvatarFrame-${uid}" src="${DEFAULT_IMG_URL}" onerror="this.src='${DEFAULT_IMG_URL}'" class="w-full h-full object-cover">
                                </div>
                        ...
                        <div class="glass-card rounded-xl p-4 relative overflow-hidden">
                            ...
                            <div class="flex items-center gap-3 bg-green-950/10 p-2.5 rounded-xl border border-green-500/5">
                                <div class="w-14 h-14 bg-black rounded-xl border border-green-500/5 overflow-hidden shrink-0">
                                     <img id="pPetImg-${uid}" src="${DEFAULT_IMG_URL}" onerror="this.src='${DEFAULT_IMG_URL}'" class="object-cover w-full h-full">
                                </div>
                        ...
                        <div id="guildLeaderContainer-${uid}" class="glass-card rounded-xl p-4 border-t-4 border-t-amber-500/60 relative overflow-hidden hidden">
                            ...
                            <div class="flex flex-col gap-3 items-center bg-green-950/5 p-2.5 rounded-xl">
                                <div class="avatar-frame w-16 h-16 relative">
                                    <img id="lAvatar-${uid}" src="${DEFAULT_IMG_URL}" onerror="this.src='${DEFAULT_IMG_URL}'" class="w-full h-full object-cover">
                                    <div class="absolute bottom-0 inset-x-0 text-center bg-black/80"><span class="text-white text-[8px] font-black font-mono">LVL <span id="lLevel-${uid}">0</span></span></div>
                                </div>
                    ...
                    const itemHtml = `
                        <div class="bg-black/60 p-1.5 rounded-xl border border-green-500/5 flex flex-col items-center justify-center">
                            <div class="w-8 h-8 mb-1 flex items-center justify-center">
                                <img src="${filterImageUrl(getImageUrl(item.item_id))}" onerror="this.src='${DEFAULT_IMG_URL}'" class="max-w-full max-h-full object-contain">
                            </div>
...
def api_status():
    with active_spam_lock:
        ...
        # Fetching start timestamps for each active spam
        active_times = {
...
def start_spam_route():
    ...
    with active_spam_lock:
        ...
        # Storing current timestamp instead of True
