...
def openroom(K, V):
    fields = {1: 2, 2: {1: 1, 2: 15, 3: 5, 4: "[FFFF00]ᴢᴇɴᴏɴ➺ꫝᴘᴏɴ TERMINAL", 5: "1", 6: 12, 7: 1, 8: 1, 9: 1, 11: 1, 12: 2, 14: 36981056, 15: {1: "IDC3", 2: 126, 3: "ME"}, 16: "\u0001\u0003\u0004\u0007\t\n\u000b\u0012\u000f\u000e\u0016\u0019\u001a \u001d", 18: 2368584, 27: 1, 34: "\u0000\u0001", 40: "en", 48: 1, 49: {1: 21}, 50: {1: 36981056, 2: 2368584, 5: 2}}}
...
def send_spam_from_all_accounts(target_id):
    ...
    for client in clients:
        if not client.online_sock or client._need_reconnect:
            ...
            client.reconnect()
            if not client.online_sock: continue
        try:
...
def spam_worker(target_id, duration_minutes):
    ...
    while True:
        with active_spam_lock:
            if target_id not in active_spam_targets: break
            if duration_minutes:
...
class FF_CLient:
    def __init__(self, uid, password):
        self.uid = uid; self.password = password
        self.key = None; self.iv = None; self.auth_token = None
        self.online_sock = None; self.running = False; self._need_reconnect = False
        self._connect()
    ...
    def _full_auth(self):
        ...
        self.key = dec.key; self.iv = dec.iv
        token = dec.token; timestamp = dec.timestamp; account_uid = dec.account_uid
        ...
        self.online_ip = online_ip; self.online_port = int(online_port)
    ...
    def _connect_online(self):
        ...
        resp = sock.recv(4096)
        if not resp: sock.close(); return None
    ...
    def _reader(self, sock):
        while self.running:
            try:
                ...
                break
        self.running = False; self._need_reconnect = True

    def _connect(self):
        if not self._full_auth():
            print(f"[-] {self.uid} Auth failed"); return
        sock = self._connect_online()
        if not sock: return
        self.online_sock = sock; self.running = True; self._need_reconnect = False
    ...
    def reconnect(self):
        ...
        self.running = False; self._connect()
...
<title>ᴢᴇɴᴏɴ➺ꫝᴘᴏɴ ULTRA TERMINAL v3.0 & FF INFO</title>
...
.neon-text-magenta { text-shadow: 0 0 10px rgba(255, 0, 127, 0.6), 0 0 20px rgba(255, 0, 127, 0.3); }
.neon-text-yellow { text-shadow: 0 0 10px rgba(234, 179, 8, 0.6), 0 0 20px rgba(234, 179, 8, 0.3); }
.green-glow-text { color: #39ff14; text-shadow: 0 0 8px rgba(57, 255, 20, 0.6), 0 0 20px rgba(57, 255, 20, 0.3); }
.stat-box-1 { background: linear-gradient(135deg, rgba(255, 0, 85, 0.1) 0%, rgba(5, 15, 28, 0.8) 100%); border: 1px solid rgba(255, 0, 85, 0.3); border-radius: 16px; text-align: center; padding: 18px 12px; box-shadow: 0 4px 20px rgba(255, 0, 85, 0.1); transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); }
.stat-box-1:hover { border-color: #ff0055; box-shadow: 0 0 25px rgba(255, 0, 85, 0.3); transform: translateY(-2px); }
.stat-box-2 { background: linear-gradient(135deg, rgba(0, 210, 255, 0.1) 0%, rgba(5, 15, 28, 0.8) 100%); border: 1px solid rgba(0, 210, 255, 0.3); border-radius: 16px; text-align: center; padding: 18px 12px; box-shadow: 0 4px 20px rgba(0, 210, 255, 0.1); transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); }
.stat-box-2:hover { border-color: #00d2ff; box-shadow: 0 0 25px rgba(0, 210, 255, 0.3); transform: translateY(-2px); }
.stat-val-1 { font-size: 2.6rem; font-weight: 800; color: #ff0055; text-shadow: 0 0 15px rgba(255, 0, 85, 0.6); line-height: 1; font-family: 'Orbitron', sans-serif; }
.stat-val-2 { font-size: 2.6rem; font-weight: 800; color: #00d2ff; text-shadow: 0 0 15px rgba(0, 210, 255, 0.6); line-height: 1; font-family: 'Orbitron', sans-serif; }
.stat-lbl { font-size: 0.8rem; text-transform: uppercase; letter-spacing: 2px; color: #a0aec0; margin-top: 8px; font-weight: 700; }
...
.cyber-link-btn { background: rgba(0, 240, 255, 0.05); border: 1px solid rgba(0, 240, 255, 0.3); color: #00f0ff; font-size: 0.75rem; font-weight: 700; letter-spacing: 1.5px; padding: 10px 18px; border-radius: 25px; transition: all 0.25s ease; display: inline-flex; align-items: center; gap: 8px; white-space: nowrap; }
...
.cyber-panel { background: rgba(15, 11, 32, 0.65); backdrop-filter: blur(16px); -webkit-backdrop-filter: blur(16px); border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 24px; padding: 26px; box-shadow: 0 20px 40px rgba(0,0,0,0.7), inset 0 1px 2px rgba(255,255,255,0.05); position: relative; }
.panel-title-bar { display: flex; align-items: center; gap: 12px; font-size: 1.2rem; font-weight: 700; letter-spacing: 1.5px; color: #00ffcc; text-shadow: 0 0 12px rgba(0, 255, 204, 0.4); margin-bottom: 22px; flex-wrap: wrap; }
...
.btn-glow-cyan { background: linear-gradient(135deg, #00f0ff 0%, #7928ca 50%, #ff0080 100%); color: #ffffff; font-weight: 700; border-radius: 30px; font-size: 1.05rem; letter-spacing: 2px; box-shadow: 0 4px 25px rgba(255, 0, 128, 0.35); transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); }
.btn-glow-cyan:hover { transform: scale(1.02); box-shadow: 0 6px 30px rgba(0, 240, 255, 0.5); }
...
.vector-card:hover { border-color: #00ffcc; box-shadow: 0 12px 35px rgba(0, 255, 204, 0.25); }
.vector-stop-btn { background: linear-gradient(135deg, #ff0055 0%, #b3003b 100%); color: #ffffff; font-size: 0.8rem; font-weight: 700; padding: 8px 16px; border-radius: 30px; box-shadow: 0 4px 12px rgba(255, 0, 85, 0.3); white-space: nowrap; transition: all 0.2s ease; }
.vector-stop-btn:hover { box-shadow: 0 6px 25px rgba(255, 0, 85, 0.7); transform: scale(1.05); }
...
.live-badge { background: linear-gradient(135deg, #ff0055, #ff7700); color: white; font-size: 0.6rem; font-weight: 800; padding: 2px 8px; border-radius: 20px; text-transform: uppercase; letter-spacing: 1px; animation: pulse-live 2s infinite; }
@keyframes pulse-live { 0%, 100% { opacity: 1; box-shadow: 0 0 5px rgba(255, 0, 85, 0.5); } 50% { opacity: 0.7; box-shadow: 0 0 15px rgba(255, 0, 85, 0.8); } }
</style>
</head>
<body class="py-6 px-4 max-w-xl mx-auto flex flex-col justify-start selection:bg-green-500 selection:text-black">

<header class="flex flex-col items-center justify-center my-4 text-center">
    <h1 class="text-3xl font-extrabold tracking-wider uppercase font-poppins bg-gradient-to-r from-cyan-400 via-pink-500 to-yellow-400 text-transparent bg-clip-text drop-shadow">ᴢᴇɴᴏɴ➺ꫝᴘᴏɴ CONTROL PANEL</h1>
    <p class="text-xs font-semibold tracking-widest text-[#a0aec0] uppercase mt-1 mb-5">Premium Cyber Infrastructure v3.0</p>
    <div class="flex flex-wrap items-center justify-center gap-3 mt-1 mb-2">
        ...
        <a href="https://t.me/BD_ADMIN_20" target="_blank" class="cyber-link-btn" style="color: #00ffcc; border-color: rgba(0, 255, 204, 0.4); background: rgba(0, 255, 204, 0.05);"><i class="fa-solid fa-address-card text-base"></i> CONTACT DEVELOPER</a>
    </div>
</header>

<div class="grid grid-cols-2 gap-4 mb-6">
    <div class="stat-box-1"><div class="stat-val-1" id="activeSpamCount">0</div><div class="stat-lbl">Active Spam</div></div>
    <div class="stat-box-2"><div class="stat-val-2" id="accCount">0</div><div class="stat-lbl">Connected Bots</div></div>
</div>

<div class="w-full mb-6">
    <button class="nav-tab w-full py-3.5 px-2 flex items-center justify-center gap-2 uppercase shadow-lg"><i class="fa-solid fa-gamepad text-sm animate-pulse"></i> Operational Core</button>
</div>
...
<div class="space-y-6">
    <div class="cyber-panel">
        ...
        <div class="space-y-4">
            ...
            <button id="startBtn" class="btn-glow-cyan w-full py-4 flex items-center justify-center gap-2 uppercase"><i class="fa-solid fa-play text-xs"></i> Start Operation</button>
        </div>
    ...
    <div class="cyber-panel border border-yellow-500/20 shadow-[0_0_20px_rgba(234,179,8,0.05)]">
        ...
        <div class="grid grid-cols-2 gap-3 text-center font-poppins">
            <div class="bg-[#0b0816] p-3 rounded-xl border border-white/5">
                <div class="text-xs text-gray-400 font-medium mb-1"><i class="fa-solid fa-eye text-emerald-400 mr-1"></i> Total Views</div>
                <span id="fbTotalViews" class="text-xl font-bold text-emerald-400 font-mono">0</span>
            </div>
            <div class="bg-[#0b0816] p-3 rounded-xl border border-white/5">
                <div class="text-xs text-gray-400 font-medium mb-1"><i class="fa-solid fa-calendar-day text-cyan-400 mr-1"></i> Today Views</div>
                <span id="fbTodayViews" class="text-xl font-bold text-cyan-400 font-mono">0</span>
            </div>
            <div class="bg-[#0b0816] p-3 rounded-xl border border-white/5">
                <div class="text-xs text-gray-400 font-medium mb-1"><i class="fa-solid fa-play text-pink-500 mr-1"></i> Total Spams</div>
                <span id="fbTotalSpamRun" class="text-xl font-bold text-pink-500 font-mono">0</span>
            </div>
            <div class="bg-[#0b0816] p-3 rounded-xl border border-white/5">
                <div class="text-xs text-gray-400 font-medium mb-1"><i class="fa-solid fa-circle-stop text-red-400 mr-1"></i> Stopped Spams</div>
...
<section class="mt-8 text-center border-t border-green-500/10 pt-4 bg-black/20 rounded-xl p-3">
    <p class="text-slate-400 font-mono text-[13px] tracking-widest mb-2.5 uppercase">Developer <span class="text-green-400 font-extrabold">ᴢᴇɴᴏɴ➺ꫝᴘᴏɴ</span></p>
...
<footer class="mt-6 mb-4 text-center text-[11px] font-semibold text-[#a0aec0] tracking-widest uppercase font-poppins">
...
const IMG_BASE_URL = 'https://cdn.jsdelivr.net/gh/ShahGCreator/icon@main/PNG/';

window.profileCache = {};
window.expandedStates = {};
window.activeSpamTimes = {};

function getImageUrl(id) { return `${IMG_BASE_URL}${id}.png`; }
...
function parseSignature(text) {
    ...
    let parsed = text
        .replace(/\[b\]/gi, '<span class="font-black">')
        .replace(/\[c\]/gi, '<div class="text-center w-full">')
...
function buildTargetCardMarkup(uid) {
    return `
        <div id="card-vector-${uid}" class="vector-card flex flex-col gap-2 relative overflow-hidden">
            <div class="flex items-center gap-2 absolute top-3 right-3 z-20">
                <span class="live-badge"><i class="fa-solid fa-circle text-[6px] mr-1"></i> LIVE</span>
            </div>
            <div class="flex flex-wrap items-center justify-between gap-3 bg-black/40 p-2.5 rounded-xl border border-white/5">
                <div class="flex items-center gap-3 min-w-[150px] flex-1">
                    <div class="w-12 h-12 rounded-xl bg-[#030207] border border-green-500/30 overflow-hidden shrink-0 relative">
                        <img id="pAvatar-${uid}" src="https://via.placeholder.com/150/020503/FFFFFF?text=Player" class="w-full h-full object-contain" />
                    </div>
                    <div class="flex flex-col font-poppins text-left">
                        <span class="text-base font-bold text-[#00ffcc] tracking-wide truncate max-w-[140px]" id="name-${uid}">Syncing Name...</span>
                ...
                <div class="flex items-center gap-2 shrink-0 flex-wrap justify-end w-full sm:w-auto mt-2 sm:mt-0">
                    <button id="view-btn-${uid}" onclick="toggleViewMore('${uid}')" class="view-more-btn flex items-center gap-1">
                        <i class="fa-solid fa-chevron-down text-[10px]"></i> View More
                    </button>
                    <button onclick="triggerStopOperation('${uid}')" class="vector-stop-btn uppercase font-poppins flex items-center gap-1">
                        <i class="fa-solid fa-hand text-[10px]"></i> STOP
                    </button>
                    <div class="w-full text-right mt-1.5 flex justify-end">
                        <span class="bg-black/60 px-2.5 py-1 rounded-md text-[11px] text-cyberGreen font-mono font-bold tracking-widest border border-green-500/20 shadow-[0_0_5px_rgba(57,255,20,0.2)]">
                            <i class="fa-solid fa-clock mr-1 text-xs"></i>
                            <span id="uptime-${uid}">0d 0h 0m</span>
                        </span>
                    </div>
                </div>
            </div>
            <div id="expanded-block-${uid}" class="hidden space-y-4 mt-2 transition-all duration-300">
                <div class="glass-card rounded-2xl relative overflow-hidden pb-4">
                    <div class="banner-wrapper">
                        <div id="bannerBg-${uid}" class="banner-bg"></div>
                        <div class="banner-overlay"></div>
                    </div>
                    <div class="basic-info">
                        <div class="avatar-frame w-[100px] h-[100px] mt-[-50px] mb-2 relative">
                            <img id="pAvatarFrame-${uid}" src="https://via.placeholder.com/150/020503/FFFFFF?text=Player" class="w-full h-full object-cover">
                        </div>
                        <div class="flex items-center gap-2 mt-1">
                            <h2 id="pFullName-${uid}" class="text-lg font-black green-glow-text font-mono truncate max-w-full">Name</h2>
                        ...
                        <div class="grid grid-cols-2 gap-3 w-full mt-4 font-mono">
                            <div class="bg-green-950/20 backdrop-blur-md rounded-xl p-2.5 border border-green-500/5 flex flex-col items-center">
                                <i class="fa-solid fa-heart text-green-400 text-lg mb-0.5 drop-shadow-[0_0_6px_rgba(57,255,20,0.4)]"></i>
                                <span class="text-[8px] text-slate-400 uppercase tracking-widest font-bold">Likes</span>
                                <span id="pLikes-${uid}" class="text-sm font-black text-white green-glow-text">0</span>
                            </div>
                            <div class="bg-green-950/20 backdrop-blur-md rounded-xl p-2.5 border border-green-500/5 flex flex-col items-center">
                                <i class="fa-solid fa-star text-emerald-400 text-lg mb-0.5 drop-shadow-[0_0_6px_rgba(0,255,102,0.4)]"></i>
                                <span class="text-[8px] text-slate-400 uppercase tracking-widest font-bold">Experience</span>
                                <span id="pExp-${uid}" class="text-sm font-black text-white green-glow-text">0</span>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="glass-card rounded-xl p-3.5 relative overflow-hidden border border-green-500/10 bg-black/40">
                    <div class="absolute top-0 left-0 w-1 h-full bg-gradient-to-b from-cyberGreen to-matrixGreen"></div>
                    <div class="text-[11px] font-bold font-mono text-slate-200 leading-relaxed whitespace-pre-wrap" id="pSignatureHtml-${uid}"></div>
                </div>
                <div class="glass-card rounded-xl p-4 relative overflow-hidden">
                    <div class="flex items-center gap-2 mb-3">
                        <i class="fa-solid fa-trophy text-amber-400 text-xs"></i>
                        <h3 class="font-black tracking-widest text-white uppercase text-[10px] font-mono">Rank Status</h3>
                    </div>
                    <div class="grid grid-cols-2 gap-2.5 font-mono">
                        <div class="bg-green-950/10 p-2.5 rounded-xl border border-green-500/5 relative overflow-hidden">
                            <span class="text-[9px] font-bold text-slate-400 block">BR Point</span>
                            <div id="pBrPoint-${uid}" class="text-base font-black text-amber-400">0</div>
                            <div class="text-[8px] text-slate-500 uppercase truncate">Max: <span id="pBrMax-${uid}">0</span></div>
                        </div>
                        <div class="bg-green-950/10 p-2.5 rounded-xl border border-green-500/5 relative overflow-hidden">
                            <span class="text-[9px] font-bold text-slate-400 block">CS Point</span>
                            <div id="pCsPoint-${uid}" class="text-base font-black green-glow-text">0</div>
                            <div class="text-[8px] text-slate-500 uppercase truncate">Max: <span id="pCsMax-${uid}">0</span></div>
                        </div>
                    </div>
                </div>
                <div class="glass-card rounded-xl p-4 relative overflow-hidden">
                    <div class="flex items-center gap-2 mb-3">
                        <i class="fa-solid fa-circle-info text-green-400 text-xs"></i>
                        <h3 class="font-black tracking-widest text-white uppercase text-[10px] font-mono">Account Metrics</h3>
                    </div>
                    <div class="grid grid-cols-2 gap-2 mb-3 font-mono text-center">
                        <div class="bg-green-950/10 p-2 rounded-xl border border-green-500/5">
                            <div class="text-[8px] text-slate-400 uppercase font-bold">Gender</div>
                            <div id="pGender-${uid}" class="text-xs font-black text-white">N/A</div>
                        </div>
                        <div class="bg-green-950/10 p-2 rounded-xl border border-green-500/5">
                            <div class="text-[8px] text-slate-400 uppercase font-bold">Language</div>
                            <div id="pLang-${uid}" class="text-xs font-black text-white">N/A</div>
                        </div>
                    </div>
                    <div class="space-y-1.5 bg-black/40 p-3 rounded-xl text-[11px] font-mono">
                        <div class="flex justify-between border-b border-green-500/5 pb-1">
                            <span class="text-slate-500">Created:</span>
                            <span id="pCreated-${uid}" class="font-bold text-slate-300">N/A</span>
                        </div>
                        <div class="flex justify-between border-b border-green-500/5 pb-1">
                            <span class="text-slate-500">Last Login:</span>
                            <span id="pLogin-${uid}" class="font-bold text-slate-300">N/A</span>
                        </div>
                        <div class="text-right pt-0.5">
                            <span class="text-[8px] text-cyberGreen font-bold uppercase">Timeline Age: <span id="pAccountAge-${uid}" class="text-white font-sans">N/A</span></span>
                        </div>
                    </div>
                </div>
                <div id="wishlistContainer-${uid}" class="glass-card rounded-xl p-4 relative overflow-hidden hidden">
                    <div class="flex items-center gap-2 mb-3">
                        <i class="fa-solid fa-gift text-green-400 text-xs"></i>
                        <h3 class="font-black tracking-widest text-white uppercase text-[10px] font-mono">Wishlist Items</h3>
                    </div>
                    <div id="wishlistGrid-${uid}" class="grid grid-cols-3 gap-2 font-mono"></div>
                    <div id="wishlistLoader-${uid}" class="flex justify-center py-2">
                        <i class="fa-solid fa-circle-notch fa-spin text-cyberGreen"></i>
                    </div>
                ...
                <div class="glass-card rounded-xl p-4 relative overflow-hidden">
                    <div class="flex items-center gap-2 mb-2">
                        <i class="fa-solid fa-gavel text-red-400 text-xs"></i>
                        <h3 class="font-black tracking-widest text-white uppercase text-[10px] font-mono">Anti-Cheat Registry Check</h3>
                    </div>
                    <div class="flex justify-between bg-green-950/10 p-2.5 rounded-xl border border-green-500/5 font-mono items-center">
                        <span class="text-slate-500 font-bold uppercase tracking-wider text-[9px]">Current Status</span>
                        <span id="pBanStatus-${uid}" class="font-black text-xs text-green-400 drop-shadow-[0_0_6px_rgba(57,255,20,0.4)]">Safe ✔</span>
                    </div>
                </div>
                <div class="glass-card rounded-xl p-4 relative overflow-hidden">
                    <div class="flex items-center gap-2 mb-3">
                        <i class="fa-solid fa-paw text-emerald-400 text-xs"></i>
                        <h3 class="font-black tracking-widest text-white uppercase text-[10px] font-mono">Active Companion</h3>
                    </div>
                    <div class="flex items-center gap-3 bg-green-950/10 p-2.5 rounded-xl border border-green-500/5">
                        <div class="w-14 h-14 bg-black rounded-xl border border-green-500/5 overflow-hidden shrink-0">
                            <img id="pPetImg-${uid}" src="https://via.placeholder.com/150/020503/FFFFFF?text=Pet" class="object-cover w-full h-full">
                        </div>
                        <div class="flex-1 font-mono text-[11px] min-w-0">
                            <div class="flex justify-between items-center mb-1 pb-1 border-b border-green-500/5">
                                <span class="text-slate-400 truncate">ID: <span id="pPetId-${uid}" class="text-white font-bold">0</span></span>
                                <span class="bg-emerald-500/10 text-emerald-400 font-black px-1.5 py-0.5 rounded text-[9px] shrink-0">LVL <span id="pPetLevel-${uid}">0</span></span>
                            </div>
                            <div class="grid grid-cols-2 gap-1 text-[9px] text-slate-500">
                                <div class="truncate">EXP: <span id="pPetExp-${uid}" class="text-white font-bold">0</span></div>
                                <div class="truncate">Skill: <span id="pPetSkill-${uid}" class="text-white font-bold">N/A</span></div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="glass-card rounded-xl p-4 relative overflow-hidden">
                    <div class="flex items-center gap-2 mb-3">
                        ...
                        <h3 class="font-black tracking-widest text-white uppercase text-[10px] font-mono">Guild Overview</h3>
                    </div>
                    <div class="space-y-2 font-mono">
                        <div class="flex justify-between items-center bg-green-950/10 p-2 rounded-xl border border-green-500/5">
                            <span id="pGuildName-${uid}" class="text-sm font-black text-white truncate uppercase max-w-[150px]">No Guild</span>
                            <span class="bg-orange-500/10 text-orange-400 text-[9px] font-black px-2 py-0.5 rounded border border-orange-500/20 shrink-0">LVL <span id="pGuildLvl-${uid}">0</span></span>
                        </div>
                        <div class="grid grid-cols-2 gap-2 text-[11px]">
                            <div class="bg-black/40 p-2 rounded-xl border border-green-500/5 overflow-hidden">
                                <div class="text-[8px] text-slate-500 uppercase font-bold truncate">Guild ID</div>
                                <div id="pGuildId-${uid}" class="text-white font-bold truncate">0</div>
                            </div>
                            <div class="bg-black/40 p-2 rounded-xl border border-green-500/5 overflow-hidden">
                                <div class="text-[8px] text-slate-500 uppercase font-bold truncate">Members</div>
                                <div class="font-black text-white truncate"><span id="pGuildMem-${uid}" class="text-cyberGreen">0</span> / <span id="pGuildCap-${uid}">0</span></div>
                            </div>
                        </div>
                    </div>
                </div>
                <div id="guildLeaderContainer-${uid}" class="glass-card rounded-xl p-4 border-t-4 border-t-amber-500/60 relative overflow-hidden hidden">
                    <div class="flex items-center gap-2 mb-3">
                        <i class="fa-solid fa-crown text-amber-500 text-xs"></i>
                        <h3 class="font-black tracking-widest text-white uppercase text-[10px] font-mono">Guild Commander</h3>
                    </div>
                    <div class="flex flex-col gap-3 items-center bg-green-950/5 p-2.5 rounded-xl">
                        <div class="avatar-frame w-16 h-16 relative">
                            <img id="lAvatar-${uid}" src="https://via.placeholder.com/150/020503/FFFFFF?text=Player" class="w-full h-full object-cover">
                            <div class="absolute bottom-0 inset-x-0 text-center bg-black/80">
                                <span class="text-white text-[8px] font-black font-mono">LVL <span id="lLevel-${uid}">0</span></span>
                            </div>
                        </div>
                        <div class="w-full text-center">
                            <h2 id="lName-${uid}" class="text-base font-black green-glow-text font-mono truncate">Name</h2>
                            <div class="text-[9px] font-mono text-slate-400 mt-0.5 truncate">UID: <span id="lUid-${uid}" class="text-white">0</span></div>
                            <div class="grid grid-cols-2 gap-1.5 font-mono text-[10px] text-left mt-3">
...
async function fetchComprehensiveData(uid) {
    try {
        ...
        if (accInfo.AccountAvatarId) {
            const avatarUrl = getImageUrl(accInfo.AccountAvatarId);
        ...
        if (accInfo.AccountBannerId && accInfo.AccountBannerId !== 0) {
            document.getElementById(`bannerBg-${uid}`).style.backgroundImage = `url('${getImageUrl(accInfo.AccountBannerId)}')`;
        }
        ...
        if (petInfo.id && petInfo.id !== 0) {
            document.getElementById(`pPetImg-${uid}`).src = getImageUrl(petInfo.id);
        }
        ...
        if (captainInfo.accountId) {
            ...
            if (captainInfo.headPic) {
                document.getElementById(`lAvatar-${uid}`).src = getImageUrl(captainInfo.headPic);
            }
        }
...
function loadOutfitAndWishlist(uid, region) {
...
async function fetchWishlistData(uid, region) {
    ...
    try {
        ...
        data.wishlist.forEach(item => {
            const itemHtml = `
                <div class="bg-black/60 p-1.5 rounded-xl border border-green-500/5 flex flex-col items-center justify-center">
                    <div class="w-8 h-8 mb-1 flex items-center justify-center">
                        <img src="${getImageUrl(item.item_id)}" class="max-w-full max-h-full object-contain" onerror="this.src='https://via.placeholder.com/64/020503/FFFFFF?text=Item'">
                    </div>
                    <div class="text-[8px] text-slate-400 font-bold truncate w-full text-center">ID: ${item.item_id}</div>
                </div>
            `;
    ...
    fetch('/api/status')
        ...
        .then(data => {
            ...
            if (data.active_spam && data.active_spam.length) {
                ...
                targetsDiv.innerHTML = '<div class="text-center text-sm text-gray-500 py-4 flex flex-col items-center justify-center gap-2"><span class="flex items-center gap-2"><i class="fa-solid fa-envelope-open opacity-40"></i> No active pipeline clusters running</span></div>';
                window.expandedStates = {};
            }
        })
...
def api_status():
    with active_spam_lock:
        ...
        active_times = {k: v['start_time'] for k, v in active_spam_targets.items()}
    with connected_clients_lock:
...
def start_spam_route():
    ...
    with active_spam_lock:
        ...
        active_spam_targets[target] = {'start_time': int(time.time())}
...
def get_wishlist():
    ...
    try:
        ...
        url = f"https://player-info-by-ckrpro.vercel.app/get?uid={uid}"
        ...
        return jsonify({'error': str(e), 'wishlist': []})
