...
# ============================================================
# ROOM SPAM AND FRIEND REQUEST BOT ENJOY
# ============================================================
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Rajdhani', sans-serif; -webkit-font-smoothing: antialiased; }
        ...
        .stat-box-3 { background: linear-gradient(135deg, rgba(57, 255, 20, 0.1) 0%, rgba(5, 15, 28, 0.8) 100%); border: 1px solid rgba(57, 255, 20, 0.3); border-radius: 16px; text-align: center; padding: 18px 12px; box-shadow: 0 4px 20px rgba(57, 255, 20, 0.1); transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); }
        ...
        .stat-val-3 { font-size: 2.6rem; font-weight: 800; color: #39ff14; text-shadow: 0 0 15px rgba(57, 255, 20, 0.6); line-height: 1; font-family: 'Orbitron', sans-serif; }
        ...
        .friend-stat-card { background: linear-gradient(135deg, rgba(57, 255, 20, 0.08) 0%, rgba(5, 15, 28, 0.8) 100%); border: 1px solid rgba(57, 255, 20, 0.2); border-radius: 16px; padding: 14px 16px; transition: all 0.3s ease; }
        .friend-stat-card:hover { border-color: #39ff14; box-shadow: 0 0 20px rgba(57, 255, 20, 0.2); }
    </style>
</head>
<body class="py-6 px-4 max-w-xl mx-auto flex flex-col justify-start selection:bg-green-500 selection:text-black">

    <div class="text-center mb-4 py-3 px-4 rounded-2xl" style="background: linear-gradient(135deg, rgba(0, 30, 0, 0.6) 0%, rgba(0, 10, 0, 0.8) 100%); border: 1px solid rgba(57, 255, 20, 0.3);">
        <h2 class="text-lg font-black tracking-widest uppercase font-poppins neon-title" style="color: #39ff14;">
            <i class="fa-solid fa-gamepad mr-2"></i>ROOM SPAM AND <i class="fa-solid fa-user-group mx-2"></i>FRIEND REQUEST BOT ENJOY <i class="fa-solid fa-earth-americas ml-2"></i>
        </h2>
    </div>
    ...
    <div class="space-y-6">
        <div class="cyber-panel">
            <div class="panel-title-bar">
                ...
                <h2 class="neon-text-magenta">ᴢᴇɴᴏɴ➺ꫝᴘᴏɴ ROOM SPAM MODE</h2>
            </div>
            <div class="space-y-4">
                ...
                <button id="startBtn" class="btn-glow-cyan w-full py-4 flex items-center justify-center gap-2 uppercase"><i class="fa-solid fa-play text-xs"></i> Start Room Spam</button>
            </div>
        ...
        <div class="cyber-panel">
            <div class="panel-title-bar">
                ...
                <i class="fa-solid fa-user-plus text-[#39ff14]"></i>
                <h2 class="green-glow-text">ᴢᴇɴᴏɴ➺ꫝᴘᴏɴ FRIEND REQUEST MODE</h2>
            </div>
            <div class="space-y-4">
                <div class="flex items-center gap-2 relative w-full">
                    <input type="text" id="friendTargetUid" class="cyber-input font-poppins text-center tracking-widest flex-1 pr-4" placeholder="Enter Target UID for Friend Request" inputmode="numeric">
                    <button id="pasteFriendUidBtn" class="cyber-paste-btn px-4 py-3.5 rounded-full text-xs font-bold font-poppins flex items-center gap-1 shrink-0 uppercase tracking-wider shadow"><i class="fa-solid fa-paste"></i> Paste</button>
                </div>
                <button id="startFriendBtn" class="btn-glow-green w-full py-4 flex items-center justify-center gap-2 uppercase"><i class="fa-solid fa-user-plus text-xs"></i> Start Friend Spam</button>
                <button id="stopFriendBtn" class="vector-stop-btn w-full py-3 flex items-center justify-center gap-2 uppercase hidden"><i class="fa-solid fa-hand text-xs"></i> Stop Friend Spam</button>
            </div>
            <div id="friendStats" class="hidden mt-4 space-y-2">
                <div class="friend-stat-card"><div class="flex justify-between items-center"><span class="text-xs text-slate-400 font-bold uppercase tracking-wider">Success</span><span id="friendSuccess" class="text-lg font-black text-green-400 font-mono">0</span></div></div>
                <div class="friend-stat-card" style="background: linear-gradient(135deg, rgba(255, 0, 85, 0.08) 0%, rgba(5, 15, 28, 0.8) 100%); border-color: rgba(255, 0, 85, 0.2);"><div class="flex justify-between items-center"><span class="text-xs text-slate-400 font-bold uppercase tracking-wider">Failed</span><span id="friendFailed" class="text-lg font-black text-red-400 font-mono">0</span></div></div>
            </div>
            <div id="friendMessage" class="toast-box bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 rounded-xl p-3 mt-3 text-sm font-medium flex items-center gap-2 font-poppins"></div>
        </div>
        ...
        <div class="cyber-panel">
            ...
            <div id="activeTargets" class="space-y-4">
                <div class="text-center text-sm text-gray-500 py-4 flex flex-col items-center justify-center gap-2"><span class="flex items-center gap-2"><i class="fa-solid fa-mailbox opacity-40"></i> No active vectors running</span></div>
            </div>
        </div>
        ...
        <div class="cyber-panel">
            ...
            <div class="panel-scroll overflow-y-auto max-h-[140px] space-y-2" id="accountList">
                <div class="text-center text-sm text-gray-500 py-4 flex items-center justify-center gap-2"><i class="fa-solid fa-circle-notch animate-spin text-xs text-[#00ffcc]"></i> Scanning cluster cores...</div>
            </div>
        </div>
        ...
        <div class="cyber-panel">
            <div class="panel-title-bar">
                ...
                <h2 class="green-glow-text">CONNECTED FRIEND REQUEST BOTS</h2>
            </div>
            <div class="panel-scroll overflow-y-auto max-h-[140px] space-y-2" id="friendAccountList">
                <div class="text-center text-sm text-gray-500 py-4 flex items-center justify-center gap-2"><i class="fa-solid fa-circle-notch animate-spin text-xs text-[#39ff14]"></i> Loading friend accounts...</div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        window.friendSpamActive = false;
        window.friendSpamTarget = null;
                ...
                <div class="flex flex-wrap items-center justify-between gap-3 bg-black/40 p-2.5 rounded-xl border border-white/5">
                    ...
                    <div class="flex items-center gap-2 shrink-0 flex-wrap justify-end w-full sm:w-auto mt-2 sm:mt-0">
                        ...
                        <div class="w-full text-right mt-1.5 flex justify-end">
                            <span class="bg-black/60 px-2.5 py-1 rounded-md text-[11px] text-cyberGreen font-mono font-bold tracking-widest border border-green-500/20 shadow-[0_0_5px_rgba(57,255,20,0.2)]"><i class="fa-solid fa-clock mr-1 text-xs"></i><span id="uptime-${uid}">0d 0h 0m</span></span>
                        </div>
                    </div>
                </div>
                <div id="expanded-block-${uid}" class="hidden space-y-4 mt-2 transition-all duration-300">
                    <div class="glass-card rounded-2xl relative overflow-hidden pb-4">
                        ...
                        <div class="basic-info">
                            <div class="avatar-frame w-[100px] h-[100px] mt-[-50px] mb-2 relative"><img id="pAvatarFrame-${uid}" src="${DEFAULT_IMG_URL}" onerror="this.src='${DEFAULT_IMG_URL}'" class="w-full h-full object-cover"></div>
                            ...
                            <div class="grid grid-cols-2 gap-3 w-full mt-4 font-mono">
                                <div class="bg-green-950/20 backdrop-blur-md rounded-xl p-2.5 border border-green-500/5 flex flex-col items-center"><i class="fa-solid fa-heart text-green-400 text-lg mb-0.5 drop-shadow-[0_0_6px_rgba(57,255,20,0.4)]"></i><span class="text-[8px] text-slate-400 uppercase tracking-widest font-bold">Likes</span><span id="pLikes-${uid}" class="text-sm font-black text-white green-glow-text">0</span></div>
                                <div class="bg-green-950/20 backdrop-blur-md rounded-xl p-2.5 border border-green-500/5 flex flex-col items-center"><i class="fa-solid fa-star text-emerald-400 text-lg mb-0.5 drop-shadow-[0_0_6px_rgba(0,255,102,0.4)]"></i><span class="text-[8px] text-slate-400 uppercase tracking-widest font-bold">Experience</span><span id="pExp-${uid}" class="text-sm font-black text-white green-glow-text">0</span></div>
                            </div>
                        </div>
                    </div>
                    <div class="glass-card rounded-xl p-3.5 relative overflow-hidden border border-green-500/10 bg-black/40"><div class="absolute top-0 left-0 w-1 h-full bg-gradient-to-b from-cyberGreen to-matrixGreen"></div><div class="text-[11px] font-bold font-mono text-slate-200 leading-relaxed whitespace-pre-wrap" id="pSignatureHtml-${uid}"></div></div>
                    <div class="glass-card rounded-xl p-4 relative overflow-hidden">
                        <div class="flex items-center gap-2 mb-3"><i class="fa-solid fa-trophy text-amber-400 text-xs"></i><h3 class="font-black tracking-widest text-white uppercase text-[10px] font-mono">Rank Status</h3></div>
                        <div class="grid grid-cols-2 gap-2.5 font-mono">
                            <div class="bg-green-950/10 p-2.5 rounded-xl border border-green-500/5 relative overflow-hidden"><span class="text-[9px] font-bold text-slate-400 block">BR Point</span><div id="pBrPoint-${uid}" class="text-base font-black text-amber-400">0</div><div class="text-[8px] text-slate-500 uppercase truncate">Max: <span id="pBrMax-${uid}">0</span></div></div>
                            <div class="bg-green-950/10 p-2.5 rounded-xl border border-green-500/5 relative overflow-hidden"><span class="text-[9px] font-bold text-slate-400 block">CS Point</span><div id="pCsPoint-${uid}" class="text-base font-black green-glow-text">0</div><div class="text-[8px] text-slate-500 uppercase truncate">Max: <span id="pCsMax-${uid}">0</span></div></div>
                        </div>
                    </div>
                    <div class="glass-card rounded-xl p-4 relative overflow-hidden">
                        <div class="flex items-center gap-2 mb-3"><i class="fa-solid fa-circle-info text-green-400 text-xs"></i><h3 class="font-black tracking-widest text-white uppercase text-[10px] font-mono">Account Metrics</h3></div>
                    ...
                    <div id="wishlistContainer-${uid}" class="glass-card rounded-xl p-4 relative overflow-hidden hidden">
                        <div class="flex items-center gap-2 mb-3"><i class="fa-solid fa-gift text-green-400 text-xs"></i><h3 class="font-black tracking-widest text-white uppercase text-[10px] font-mono">Wishlist Items</h3></div>
                    ...
                    <div class="glass-card rounded-xl p-4 relative overflow-hidden">
                        ...
                        <div class="flex justify-between bg-green-950/10 p-2.5 rounded-xl border border-green-500/5 font-mono items-center"><span class="text-slate-500 font-bold uppercase tracking-wider text-[9px]">Current Status</span><span id="pBanStatus-${uid}" class="font-black text-xs text-green-400 drop-shadow-[0_0_6px_rgba(57,255,20,0.4)]">Safe ✔</span></div>
                    </div>
                    <div class="glass-card rounded-xl p-4 relative overflow-hidden">
                        <div class="flex items-center gap-2 mb-3"><i class="fa-solid fa-paw text-emerald-400 text-xs"></i><h3 class="font-black tracking-widest text-white uppercase text-[10px] font-mono">Active Companion</h3></div>
                        <div class="flex items-center gap-3 bg-green-950/10 p-2.5 rounded-xl border border-green-500/5">
                            <div class="w-14 h-14 bg-black rounded-xl border border-green-500/5 overflow-hidden shrink-0"><img id="pPetImg-${uid}" src="${DEFAULT_IMG_URL}" onerror="this.src='${DEFAULT_IMG_URL}'" class="object-cover w-full h-full"></div>
                    ...
                    <div class="glass-card rounded-xl p-4 relative overflow-hidden">
                        <div class="flex items-center gap-2 mb-3"><i class="fa-solid fa-shield-halved text-orange-400 text-xs"></i><h3 class="font-black tracking-widest text-white uppercase text-[10px] font-mono">Guild Overview</h3></div>
                    ...
                    <div id="guildLeaderContainer-${uid}" class="glass-card rounded-xl p-4 border-t-4 border-t-amber-500/60 relative overflow-hidden hidden">
                        <div class="flex items-center gap-2 mb-3"><i class="fa-solid fa-crown text-amber-500 text-xs"></i><h3 class="font-black tracking-widest text-white uppercase text-[10px] font-mono">Guild Commander</h3></div>
                        <div class="flex flex-col gap-3 items-center bg-green-950/5 p-2.5 rounded-xl">
                            <div class="avatar-frame w-16 h-16 relative"><img id="lAvatar-${uid}" src="${DEFAULT_IMG_URL}" onerror="this.src='${DEFAULT_IMG_URL}'" class="w-full h-full object-cover"><div class="absolute bottom-0 inset-x-0 text-center bg-black/80"><span class="text-white text-[8px] font-black font-mono">LVL <span id="lLevel-${uid}">0</span></span></div></div>
        ...
        function fetchStatus() {
            fetch('/api/status')
                ...
                .then(data => {
                    ...
                    if (data.friend_spam_stats) {
                        const stats = data.friend_spam_stats;
                        const target = Object.keys(stats)[0];
                        if (target) {
                            document.getElementById('friendSuccess').innerText = stats[target].success || 0;
                            document.getElementById('friendFailed').innerText = stats[target].failed || 0;
                        }
                    }
                    const accListDiv = document.getElementById('accountList');
                    if (data.accounts && data.accounts.length) {
                        accListDiv.innerHTML = data.accounts.map(acc => 
                            `<div class="text-xs bg-[#0d091f] border border-purple-900/40 px-4 py-3 rounded-full text-gray-300 flex items-center justify-between font-poppins"><span class="flex items-center gap-2"><span class="w-1.5 h-1.5 rounded-full bg-[#00ffcc] shadow-[0_0_6px_#00ffcc]"></span> ᴢᴇɴᴏɴ➺ꫝᴘᴏɴ_NODE</span><span class="text-purple-300 font-mono font-medium">${acc}</span></div>`
                        ).join('');
                    } else {
        ...
        document.getElementById('pasteFriendUidBtn').onclick = async () => {
            playClickSound();
            try {
                const text = await navigator.clipboard.readText();
                if(text) {
                    const cleanText = text.trim();
                    if(/^\d+$/.test(cleanText)) {
                        document.getElementById('friendTargetUid').value = cleanText;
                        showToast("UID successfully pasted!");
                    } else {
                        showToast("Clipboard text is not a valid UID numeric!");
                    }
        ...
        document.getElementById('startFriendBtn').onclick = () => {
            const uid = document.getElementById('friendTargetUid').value.trim();
            if (!uid) {
                showToastNotification('friendMessage', 'Please enter target UID for friend request!', true);
                return;
            }
            if (!/^\d+$/.test(uid)) {
                showToastNotification('friendMessage', 'UID must be numeric!', true);
                return;
            }
            fetch(`/start_friend_spam?uid=${encodeURIComponent(uid)}`)
                .then(res => res.json())
                .then(data => {
                    if (data.error) {
                        showToastNotification('friendMessage', data.error, true);
                    } else {
                        showToastNotification('friendMessage', 'Friend Spam Started Successfully!');
                        document.getElementById('friendTargetUid').value = '';
                        document.getElementById('startFriendBtn').classList.add('hidden');
                        document.getElementById('stopFriendBtn').classList.remove('hidden');
                        document.getElementById('friendStats').classList.remove('hidden');
                        window.friendSpamActive = true;
                        window.friendSpamTarget = uid;
                        fetchStatus();
                    }
                })
                .catch(err => showToastNotification('friendMessage', 'Server Transmission Failed', true));
        };
        document.getElementById('stopFriendBtn').onclick = () => {
            const uid = window.friendSpamTarget;
            if (!uid) return;
            fetch(`/stop_friend_spam?uid=${encodeURIComponent(uid)}`)
                .then(res => res.json())
                .then(data => {
                    if (data.error) {
                        showToastNotification('friendMessage', data.error, true);
                    } else {
                        showToastNotification('friendMessage', 'Friend Spam Stopped!');
                        document.getElementById('startFriendBtn').classList.remove('hidden');
                        document.getElementById('stopFriendBtn').classList.add('hidden');
                        document.getElementById('friendStats').classList.add('hidden');
                        window.friendSpamActive = false;
                        window.friendSpamTarget = null;
                        fetchStatus();
                    }
                })
                .catch(err => showToastNotification('friendMessage', 'Stop Request Failed', true));
        };
        setInterval(() => {
...
@app.route('/start_friend_spam')
def start_friend_spam_route():
    target = request.args.get('uid')
    if not target:
        return jsonify({'error': 'uid parameter required'}), 400
    with friend_accounts_lock:
        if not friend_accounts:
            return jsonify({'error': 'No friend accounts loaded from friend.txt'}), 500
    with friend_spam_stats_lock:
        if target in friend_spam_running:
            return jsonify({'error': f'Friend spam already running for {target}'}), 409
    threading.Thread(target=Friend_SpamWorker, args=(target,), daemon=True).start()
    return jsonify({
        'status': 'Friend spam started',
        'target': target
    })

@app.route('/stop_friend_spam')
def stop_friend_spam_route():
    target = request.args.get('uid')
    if not target:
        return jsonify({'error': 'uid parameter required'}), 400
    if target in friend_spam_running:
        friend_spam_running[target] = False
        return jsonify({'status': f'Friend spam stopped for {target}'})
    else:
        return jsonify({'error': f'No friend spam running for {target}'}), 404

if __name__ == '__main__':
    # Load friend accounts
