...
# main.py এর মতো নতুন ও কার্যকরী লগইন ইউআরএল এবং অবজেক্ট সেটআপ
login_url = "https://loginbp.ggblueshark.com"
ob = "OB53"
version = "1.123.1"

# main.py এর নতুন Headers充নফিগারেশন
Hr = {
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(url, headers=headers, data=data, timeout=15) as response:
                    ...
                    print(f"[-] Attempt {attempt} failed (Status: {response.status}). Retrying in 5 seconds...")
        except Exception as e:
            print(f"[-] Network Error on attempt {attempt}: {e}. Retrying in 5 seconds...")
                        ...
                        if self.response:
                            ...
                            if self.inPuTMsG.startswith(("/s5")):
                                try:
                                    ...
                                    PAc = await OpEnSq(key , iv, self.region)
                                    await self.SEndPacKeT(self.whisper_writer , self.online_writer , 'OnLine' , PAc)

                                    C = await cHSq(5, uid ,key, iv, self.region)
                                    await asyncio.sleep(0.5)
                                    await self.SEndPacKeT(self.whisper_writer , self.online_writer , 'OnLine' , C)
                                    V = await SEnd_InV(5 , uid , key , iv, self.region)
                                    await asyncio.sleep(0.5)
                            ...
                            # ---- @a স্প্যামিং অপ্টিমাইজড সেকশন (Concurrency Added) ----
                                ...
                                try:
                                    ...
                                    if not s:
                                        try:
                                            ...
                                            # সব আইডিতে একসাথে প্যারালালে ইনভাইট/ইমোট পাঠানোর জন্য টাস্ক লিস্ট
                                            tasks = []
                                            
                                            async def fire_packet(target_uid):
                                                try:
                                                    H = await Emote_k(target_uid, idT, key, iv, self.region)
                                                    await self.SEndPacKeT(self.whisper_writer, self.online_writer, 'OnLine', H)
                                                except Exception:
                                                    pass

                                            # যে যে আইডিগুলো পাওয়া গেছে তাদের একসাথে ফায়ার করার প্রস্তুতি
                                            if uid: tasks.append(fire_packet(uid))
                                            if uid2: tasks.append(fire_packet(uid2))
                                            if uid3: tasks.append(fire_packet(uid3))
                                            if uid4: tasks.append(fire_packet(uid4))
                                            if uid5: tasks.append(fire_packet(uid5))
                                                
                                            # gather এর মাধ্যমে সবগুলো আইডি এক সেকেন্ডে একসাথে ইনভাইট করবে
                                            if tasks:
